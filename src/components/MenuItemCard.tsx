...
import { Plus, Check, Clock, Flame, Star, Eye } from 'lucide-react';
...
export const MenuItemCard: React.FC<MenuItemCardProps> = ({
  ...
  if (viewMode === 'list') {
    return (
      <div
        ...
        className="group relative flex flex-col sm:flex-row items-stretch sm:items-center justify-between p-4 bg-white hover:bg-stone-50/80 rounded-sm border border-stone-200 hover:border-stone-400 transition-all duration-150 cursor-pointer shadow-xs gap-4"
      >
        {/* Left: Thumbnail & Details */}
        <div className="flex items-center space-x-4 flex-1 min-w-0">
          <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-xs overflow-hidden shrink-0 bg-stone-100 border border-stone-200">
            <img
              src={item.image}
              alt={item.name}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <span className="text-[11px] uppercase tracking-wider text-[#a83b24] font-bold">
                {item.category === 'entradas' ? 'Entrada' : item.category === 'platos_fuertes' ? 'Plato Fuerte' : 'Postre'}
              </span>
        ...
        <div className="flex items-center justify-between sm:flex-col sm:items-end sm:justify-center border-t sm:border-t-0 border-stone-200 pt-2 sm:pt-0 shrink-0 gap-2">
          <div className="text-left sm:text-right">
            <span className="text-[10px] uppercase tracking-wider text-stone-400 block font-medium">Precio</span>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-3.5 py-2 rounded-sm text-xs font-semibold uppercase tracking-wider transition-all cursor-pointer shadow-xs ${
                justAdded
                  ? 'bg-emerald-700 text-white'
                  : 'bg-[#a83b24] hover:bg-[#91321d] text-white active:scale-95'
              }`}
            >
              {justAdded ? (
                <>
  ...
  return (
    <div
      ...
      onClick={() => onSelect(item)}
      className="group relative flex flex-col bg-white hover:bg-stone-50/50 rounded-sm border border-stone-200 hover:border-stone-400 transition-all duration-200 overflow-hidden cursor-pointer shadow-xs hover:shadow-md"
    >
      {/* Top Media: High Quality Dish Photo */}
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-stone-200">
          ...
          <button
            id={`grid-add-${item.id}`}
            onClick={handleQuickAdd}
            className={`flex items-center space-x-1.5 px-4 py-2 rounded-sm text-xs font-semibold uppercase tracking-wider transition-all duration-150 shadow-xs cursor-pointer ${
              justAdded
                ? 'bg-emerald-700 text-white'
                : 'bg-[#a83b24] hover:bg-[#91321d] text-white active:scale-95'
            }`}
            title="Añadir a mi comanda"
          >
            {justAdded ? (
              <>
                <Check className="w-3.5 h-3.5 stroke-[3]" />
